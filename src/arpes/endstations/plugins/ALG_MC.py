...
from arpes.endstations import HDF5Endstation, FITSEndstation, HemisphericalEndstation
from arpes.constants import pi

__all__ = ("ALGMainChamber",)


class ALGMainChamber(HemisphericalEndstation, HDF5Endstation):
    """Implements data loading for the Lanzara group "Main Chamber"."""
...
class ALGMainChamberFITS(HemisphericalEndstation, FITSEndstation):
    """Implements data loading for the Lanzara group "Main Chamber"."""

    PRINCIPAL_NAME = "ALG-Main_fits"
    ALIASES = [
        "MC_fits",
        "ALG-Main_fits",
        "ALG-MC_fits",
        "ALG-Hemisphere_fits",
        "ALG-Main Chamber_fits",
    ]
    ATTR_TRANSFORMS = {
        "START_T": lambda l: {
            "time": " ".join(l.split(" ")[1:]).lower(),
